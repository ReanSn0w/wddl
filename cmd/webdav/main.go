...
var (
	...
	opts     = struct {
		...
		PSize   int64  `long:"p-size" env:"P_SIZE" default:"1024" description:"part size (kb)"`
		DBFile  string `long:"db-file" env:"DB_FILE" default:"./wddl.db" description:"database file"`
		Threads int    `long:"threads" env:"THREADS" default:"10" description:"parallel downloads"`
...
func main() {
...

	{
		...
		engine := engine.New(app.Log(), config, queue, files, files, files)
